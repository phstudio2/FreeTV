...
class CzechRepublic : AppCompatActivity() {

    @Suppress("DEPRECATION")
    private fun internet(): Boolean {
        val connectivityManager =
            this@CzechRepublic.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_czech_republic)

        val recyclerView: RecyclerView = findViewById(R.id.rvCzechRepublic)
        customAdapter =
            ItemAdapter2(
                this@CzechRepublic,
                czechRepublicList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(czechRepublicList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@CzechRepublic, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@CzechRepublic, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_czech_republic.m3u8")
                    ...
                    val czechRepublicToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        czechRepublicList.addAll(czechRepublicToAdd)
    ...
    private fun splitList(czechRepublicList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in czechRepublicList) {
    ...
    private val czechRepublicList = ArrayList<Triple<String, String, String>>()
