...
class Belgium : AppCompatActivity() {
    ...
    private fun internet(): Boolean {
        val connectivityManager =
            this@Belgium.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_belgium)

        val recyclerView: RecyclerView = findViewById(R.id.rvBelgium)
        customAdapter =
            ItemAdapter2(
                this@Belgium,
                belgiumList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(belgiumList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@Belgium, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@Belgium, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_belgium.m3u8")
                    ...
                    val belgiumToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        belgiumList.addAll(belgiumToAdd)
    ...
    private fun splitList(belgiumList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in belgiumList) {
    ...
    private val belgiumList = ArrayList<Triple<String, String, String>>()
