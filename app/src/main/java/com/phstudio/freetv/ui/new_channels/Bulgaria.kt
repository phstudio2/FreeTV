...
class Bulgaria : AppCompatActivity() {

    @Suppress("DEPRECATION")
    private fun internet(): Boolean {
        val connectivityManager =
            this@Bulgaria.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_bulgaria)

        val recyclerView: RecyclerView = findViewById(R.id.rvBulgaria)
        customAdapter =
            ItemAdapter2(
                this@Bulgaria,
                bulgariaList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(bulgariaList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@Bulgaria, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@Bulgaria, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_bulgaria.m3u8")
                    ...
                    val bulgariaToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        bulgariaList.addAll(bulgariaToAdd)
    ...
    private fun splitList(bulgariaList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in bulgariaList) {
    ...
    private val bulgariaList = ArrayList<Triple<String, String, String>>()
