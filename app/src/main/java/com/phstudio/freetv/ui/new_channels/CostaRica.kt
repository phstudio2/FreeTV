...
class CostaRica : AppCompatActivity() {

    @Suppress("DEPRECATION")
    private fun internet(): Boolean {
        val connectivityManager =
            this@CostaRica.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_costa_rica)

        val recyclerView: RecyclerView = findViewById(R.id.rvCostaRica)
        customAdapter =
            ItemAdapter2(
                this@CostaRica,
                costaRicaList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(costaRicaList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@CostaRica, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@CostaRica, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_costa_rica.m3u8")
                    ...
                    val costaRicaToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        costaRicaList.addAll(costaRicaToAdd)
    ...
    private fun splitList(costaRicaList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in costaRicaList) {
    ...
    private val costaRicaList = ArrayList<Triple<String, String, String>>()
