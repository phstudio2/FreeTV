...
class Albania : AppCompatActivity() {
    ...
    private fun internet(): Boolean {
        val connectivityManager =
            this@Albania.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_albania)

        val recyclerView: RecyclerView = findViewById(R.id.rvAlbania)
        customAdapter =
            ItemAdapter2(
                this@Albania,
                albaniaList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(albaniaList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@Albania, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@Albania, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_albania.m3u8")
                    ...
                    val albaniaToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        albaniaList.addAll(albaniaToAdd)
    ...
    private fun splitList(albaniaList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in albaniaList) {
    ...
    private val albaniaList = ArrayList<Triple<String, String, String>>()
