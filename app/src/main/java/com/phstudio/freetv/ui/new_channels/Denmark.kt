...
class Denmark : AppCompatActivity() {
    ...
    private fun internet(): Boolean {
        val connectivityManager =
            this@Denmark.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_denmark)

        val recyclerView: RecyclerView = findViewById(R.id.rvDenmark)
        customAdapter =
            ItemAdapter2(
                this@Denmark,
                denmarkList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(denmarkList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@Denmark, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@Denmark, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
    ...
    private fun getPublic() {
        if (internet()) {
            Thread {
                try {
                    val website =
                        URL("https://raw.githubusercontent.com/Free-TV/IPTV/master/playlists/playlist_denmark.m3u8")
                    ...
                    val denmarkToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        denmarkList.addAll(denmarkToAdd)
    ...
    private fun splitList(denmarkList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in denmarkList) {
    ...
    private val denmarkList = ArrayList<Triple<String, String, String>>()
