...
class KodiTvMain : AppCompatActivity() {

    @Suppress("DEPRECATION")
    private fun internet(): Boolean {
        val connectivityManager =
            this@KodiTvMain.getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.layout_kodi_tv_main)

        val recyclerView: RecyclerView = findViewById(R.id.rvKodiTvMain)
        customAdapter =
            ItemAdapter2(
                this@KodiTvMain,
                kodiTvMainList,
                object : ItemAdapter2.OnItemClickListener {
                    override fun onItemClick(position: Int) {
                        val (_, _, stringList2) = splitList(kodiTvMainList)
                        val url = stringList2[position]
                        if (url.contains("https://www.youtube.com")) {
                            val intent = Intent(this@KodiTvMain, HTMLActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        } else {
                            val intent = Intent(this@KodiTvMain, PlayerActivity::class.java)
                            intent.putExtra("Name", url)
                            startActivity(intent)
                        }
        ...
        if (internet()) {
            Thread {
                try {
                    ...
                    val kodiTvMainToAdd = ArrayList<Triple<String, String, String>>()
                    ...
                    runOnUiThread {
                        kodiTvMainList.addAll(kodiTvMainToAdd)
    ...
    private fun splitList(kodiTvMainList: ArrayList<Triple<String, String, String>>): Triple<ArrayList<String>, ArrayList<String>, ArrayList<String>> {
        val stringList1 = ArrayList<String>()
        val intList = ArrayList<String>()
        val stringList2 = ArrayList<String>()

        for (pair in kodiTvMainList) {
